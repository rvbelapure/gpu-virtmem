...
/**
 * The implementation of new cubin (cuda 4.0) format marshalling
 * 
 * 
 * @return cuda_err set to cudaErrorMemoryAllocation if I cannot calloc the packet
 *
 */
void** r__cudaRegisterFatBinary_4(void* fatC, int index){
	cuda_packet_t * pPacket;
	// here we will store the number of entries to spare counting again and again
	// @todo might be unimportant
	cache_num_entries_t entries_cached = {0, 0, 0, 0, 0, 0, 0};
	...
	// the original cubin to get rid of casting to __cudaFatCudaBinary2
	__cudaFatCudaBinary2 * pSrcFatC = (__cudaFatCudaBinary2 *)fatC;

	if(pSrcFatC->magic == __cudaFatMAGIC2)
		fprintf(stderr, "cuda version 4+\n");
	// the place where the packed fat binary will be stored
	char * pPackedFat = NULL;
	...
	fb_size = getFatRecPktSize_4(pSrcFatC, &entries_cached);
	...
	if( packFatBinary_4(pPackedFat, pSrcFatC, &entries_cached) == ERROR ){
		exit(ERROR);
	}
...
void** __cudaRegisterFatBinary(void* fatC) {
  ...
  if(DONE==1)
  {
   ...
   DONE=2;
    ...
    fprintf(stderr, "cuda 4.0 cubin\n");
    while(index<BACKEND_NODES)
    {
        fatcubinhandle_array[fatcubin_handle_counter-1][index]=r__cudaRegisterFatBinary_4(fatC, index);
        index++;
    }
    } 
    return(ret);
...
void r__cudaUnregisterFatBinary2(void** fatCubinHandle, int index) {
	cuda_packet_t * pPacket;
