...
cudaError_t cudaThreadSynchronize(void) {
    ...
    if(CUR_DEV<DEVICES_PER_NODE)
        ...
        ret = rcudaThreadSynchronize(CUR_DEV/DEVICES_PER_NODE);
    return(ret);
}
...
cudaError_t cudaSetDevice(int device) {
    ...
    if(device<DEVICES_PER_NODE)
        ...
        ret = rcudaSetDevice(device%DEVICES_PER_NODE, device/DEVICES_PER_NODE);
    if(ret==cudaSuccess)
        CUR_DEV = device;
    return(ret);
}
...
printf("IN cudaConfigureCall************\n");

    if(CUR_DEV<DEVICES_PER_NODE)
        ret = lcudaConfigureCall(gridDim, blockDim, sharedMem, stream);
    else
        ret = rcudaConfigureCall(gridDim, blockDim, sharedMem, stream, CUR_DEV/DEVICES_PER_NODE);
    return(ret);
}
...
cudaError_t cudaSetupArgument(const void *arg, size_t size, size_t offset) {
    ...
    if(CUR_DEV<DEVICES_PER_NODE)
        ...
        ret = rcudaSetupArgument(arg, size, offset, CUR_DEV/DEVICES_PER_NODE);
    return(ret);
}
...
cudaError_t cudaLaunch(const char *entry) {
    ...
    if(CUR_DEV<DEVICES_PER_NODE)
        ...
        ret = rcudaLaunch(entry, CUR_DEV/DEVICES_PER_NODE);
    return(ret);
}
...
cudaError_t cudaMalloc(void **devPtr, size_t size) {
    ...
    if (CUR_DEV<DEVICES_PER_NODE)
        ...
        ret = rcudaMalloc(devPtr, size, CUR_DEV/DEVICES_PER_NODE);
    return(ret);
}
...
cudaError_t cudaFree(void * devPtr) {
    ...
    if(CUR_DEV<DEVICES_PER_NODE)
        ...
        ret = rcudaFree(devPtr, CUR_DEV/DEVICES_PER_NODE);
    return(ret);
}
...
cudaError_t cudaMemcpy(void *dst, const void *src, size_t count,
    ...
    if(CUR_DEV<DEVICES_PER_NODE)
        ...
        ret = rcudaMemcpy(dst, (const void *) src, count, kind, CUR_DEV/DEVICES_PER_NODE);
    return(ret);
}
...
void __cudaRegisterFunction(void** fatCubinHandle, const char* hostFun,
    ...
    while(index < BACKEND_NODES)
    {
        r__cudaRegisterFunction(
                                r__cudaRegisterFatBinary(fatC_global, index),hostFun, deviceFun,
                                deviceName, thread_limit, tid, bid,  bDim,  gDim, wSize, index
                                );
        index++;
    }
